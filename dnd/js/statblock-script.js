...
function UpdateStatblock(moveSeparationPoint) {
    ...
    let separationMax = mon.abilities.length + mon.actions.length + mon.additionalactions.length + mon.reactions.length - 1;
    ...
    $("#monster-type").html(StringFunctions.StringCapitalize(mon.type + " " + mon.size +
    ...
    setPts("#fuepts", mon.fuePoints);
    setPts("#despts", mon.desPoints);
    ...
    if (mon.isLegendary)
        AddToTraitList(traitsHTML, mon.legendaries, mon.legendariesDescription == "" ? "<h3>Acciones legendarias</h3>" : ["<h3>Acciones legendarias</h3><div class='property-block'>", ReplaceTags(StringFunctions.RemoveHtmlTags(mon.legendariesDescription)), "</div></br>"], true);
    ...
    if (mon.isMythic && mon.isLegendary && (mon.mythics.length > 0 || mon.mythicDescription.length > 0)) {
        AddToTraitList(traitsHTML, mon.mythics, mon.mythicDescription == "" ? "<h3>Acciones Míticas</h3><div class='property-block'></div>" : ["<h3>Acciones Míticas</h3><div class='property-block'>", StringFunctions.FormatString(ReplaceTags(StringFunctions.RemoveHtmlTags(mon.mythicDescription))), "</div></br>"], true);
    }
...
function AddToTraitList(traitsHTML, traitsArr, addElements, isLegendary = false, isLairRegional = false, isMythic = false) {
    ...
    for (let index = 0; index < traitsArr.length; index++) {
        if (isLegendary) {
            ...
            traitsHTML.push(StringFunctions.MakeTraitHTMLMythic(traitsArr[index].name, ReplaceTags(traitsArr[index].desc)));
        } else {
...
function TryMarkdown() {
    ...
    markdown.push(', ', mon.alignment, '*<br>>___<br>> - **Armor Class** ', StringFunctions.FormatString(StringFunctions.GetArmorData()), '<br>> - **Hit Points** ', StringFunctions.GetHP(), '<br>> - **Speed** ', StringFunctions.GetSpeed(), "<br>>___<br>>|FUE|DES|CON|INT|SAB|CAR|<br>>|:---:|:---:|:---:|:---:|:---:|:---:|<br>>|",
        mon.fuePoints, " (", StringFunctions.BonusFormat(MathFunctions.PointsToBonus(mon.fuePoints)), ")|",
        mon.desPoints, " (", StringFunctions.BonusFormat(MathFunctions.PointsToBonus(mon.desPoints)), ")|",
        ...
        mon.sabPoints, " (", StringFunctions.BonusFormat(MathFunctions.PointsToBonus(mon.sabPoints)), ")|",
        mon.carPoints, " (", StringFunctions.BonusFormat(MathFunctions.PointsToBonus(mon.carPoints)), ")|<br>>___<br>");
    ...
    if (mon.additionalactions.length > 0) markdown.push("<br>> ### Additional Actions<br>", GetTraitMarkdown(mon.additionalactions, false));
    if (mon.reactions.length > 0) markdown.push("<br>> ### Reactions<br>", GetTraitMarkdown(mon.reactions, false));
    if (mon.isLegendary) {
        markdown.push("<br>> ### Acciones legendarias<br>> ", ReplaceTags(mon.legendariesDescription));
        if (mon.legendaries.length > 0) markdown.push("<br>><br>", GetTraitMarkdown(mon.legendaries, true));
    }
    if (mon.isMythic) {
        markdown.push("<br>> ### Acciones Míticas<br>> ", ReplaceTags(mon.mythicDescription));
        if (mon.mythics.length > 0) markdown.push("<br>><br>", GetTraitMarkdown(mon.mythics, false, true));
    }
    if (mon.isLair && mon.isLegendary) {
...
var FormFunctions = {
    ...
    SetForms: function () {
        ...
        // Stats
        this.SetStatForm("fue", mon.fuePoints);
        this.SetStatForm("des", mon.desPoints);
        this.SetStatForm("con", mon.conPoints);
        this.SetStatForm("int", mon.intPoints);
        this.SetStatForm("sab", mon.sabPoints);
        this.SetStatForm("car", mon.carPoints);
        ...
        // Is Mythic?	
        $("#has-mythic-input").prop("checked", mon.isMythic);
        this.ShowHideMythic();
    ...
    ShowHideOtherArmor: function () {
        ...
        if ($("#armor-input").val() == "armadura natural")
            $("#natarmor-prompt").show();
        else if ($("#armor-input").val() == "otro")
    ...
    ShowHideMagicDarknessBox: function () {
        this.ShowHideHtmlElement("#magic-darkness-box-note", $("#darkvision-input").val() > 0);
    },
    ...
    ShowHideLegendaryCreature: function () {
        if ($("#is-legendary-input:checked").val()) {
            ...
            if ($("#has-mythic-input:checked").val())
    ...
    ShowHideMythic: function () {
        $("#has-mythic-input:checked").val() ?
            $("#add-mythic-button, #mythic-actions-form").show() :
            $("#add-mythic-button, #mythic-actions-form").hide();
    },
    ...
    // For setting the mythic effect end description
    SetMythicDescriptionEndForm: function () {
        $("#mythic-end-descsection-input").val(mon.mythicDescriptionEnd);
    },
    ...
    InitForms: function () {
        let dropdownBuffer = [
            ...
            "<option value=0>0 (", data.crs["0"].pe, " PE)</option>",
            "<option value=1/8>1/8 (", data.crs["1/8"].pe, " PE)</option>",
            "<option value=1/4>1/4 (", data.crs["1/4"].pe, " PE)</option>",
            "<option value=1/2>1/2 (", data.crs["1/2"].pe, " PE)</option>"
        ];
        for (let cr = 1; cr <= 30; cr++)
            dropdownBuffer.push("<option value=", cr, ">", cr, " (", data.crs[cr].pe, " PE)</option>");
...
var GetVariablesFunctions = {
    ...
    GetAllVariables: function () {
        ...
        // Speeds
        mon.speed = parseInt($("#speed-input").val());
        mon.burrowSpeed = parseInt($("#burrow-speed-input").val());
        mon.climbSpeed = parseInt($("#climb-speed-input").val());
        mon.flySpeed = parseInt($("#fly-speed-input").val());
        mon.hover = $("#hover-input").prop("checked");
        mon.swimSpeed = parseInt($("#swim-speed-input").val());
        mon.speedDesc = $("#custom-speed-prompt").val();
        mon.customSpeed = $("#custom-speed-input").prop("checked");

        // Stats	
        mon.fuePoints = $("#fue-input").val();
        mon.desPoints = $("#des-input").val();
        ...
        // Mythic
        mon.isMythic = $("#has-mythic-input").prop("checked");
        if (mon.isMythic) {
    ...
    SetPreset: function (preset) {
        ...
        if (armorDescData) {
            ...
            if (armorDescData.length == 1 && armorDescData[0].trim() == "escudo") {
                mon.shieldBonus = 2;
                mon.armorName = "none";
            } else {
                // If we have a shield in addition to something else
                if (armorDescData.length > 1) {
                    if (armorDescData[1].trim() == "escudo") {
                        mon.shieldBonus = 2;
                        mon.armorName = armorDescData[0];
                    }
                ...
                // Is it natural armor?
                if (mon.armorName == "armadura natural") {
                ...
                // Is it mage armor?
                else if (mon.armorName.includes("armadura de mago"))
                    mon.armorName = "armadura de mago";
            ...
            mon.armorName = (armorAcData == MathFunctions.GetAC("none") ? "none" : "otro");

        // In case it's an unknown armor type
        if (mon.armorName == "otro") {
            if (armorDescData)
                mon.otherArmorDesc = armorDescData[0].includes("(") ? armorDescData :
                    armorAcData + " (" + armorDescData + ")";
            else
                mon.otherArmorDesc = armorAcData + " (tipo de armadura desconocido)";
        ...
        if (preset.speed.hasOwnProperty("notes")) {
            ...
            mon.speedDesc = preset.speed.walk + " pies (" + preset.speed.notes + ")";
        } else {
        ...
        mon.telepathy = 0;
        mon.understandsBut = "";
        if (preset.languages.includes("entiende")) {
            let speaksUnderstandsArr = preset.languages.split("entiende"),
                speaks = speaksUnderstandsArr[0].length > 0 ? speaksUnderstandsArr[0].trim().split(",") : [],
                understands = speaksUnderstandsArr[1].split(" pero "),
                understandsLangs = understands[0].replace(", y ", ",").replace(" y ", ",").split(","),
            ...
            if (understandsBut.toLowerCase().includes("telepatía")) {
            ...
            for (let index = 0; index < languagesPresetArr.length; index++) {
                ...
                languageName.toLowerCase().includes("telepatía") ?
        ...
        for (let index = 0; index < sensesPresetArr.length; index++) {
            ...
            switch (senseName) {
                case "blindsight":
                    ...
                    mon.blind = senseString.toLowerCase().includes("ciego más allá");
                    break;
                case "darkvision":
                    mon.darkvision = senseDist;
                    mon.magicdarkness = senseString.toLowerCase().includes("penetra oscuridad mágica");
                    break;
                case "tremorsense":
        ...
        let abilitiesPresetArr = preset.special_abilities,
            ...
            additionalactionsPresetArr = preset.additionalactions,
            reactionsPresetArr = preset.reactions,
            legendariesPresetArr = preset.legendary_actions,
            mythicsPresetArr = preset.mythic_actions,
        ...
        AbilityPresetLoop(additionalactionsPresetArr, "additionalactions");
        AbilityPresetLoop(reactionsPresetArr, "reactions");
        if (mon.isLegendary)
            AbilityPresetLoop(legendariesPresetArr, "legendaries");
        if (mon.isMythic)
            AbilityPresetLoop(mythicsPresetArr, "mythics");
        if (mon.isLair)
    ...
    AddLanguage: function (languageName, speaks) {
        ...
        if (mon.languages.length > 0) {
            if (languageName.toLowerCase() == "todos" || mon.languages[0].name.toLowerCase() == "todos")
                mon.languages = [];
        }
    ...
    // Add abilities, actions, additionalactions, reactions, and legendary actions, etc
        ...
        if (arrName == "abilities" && abilityName.toLowerCase().includes("spellcasting") && abilityDesc.includes("\n")) {
            abilityDesc = abilityDesc.split("\u2022").join(""), // Remove bullet points
                ...
                abilityDesc.toLowerCase().includes("intelligence") ? "INT" :
                    abilityDesc.toLowerCase().includes("wisdom") ? "SAB" :
                        abilityDesc.toLowerCase().includes("charisma") ? "CAR" : null;
    ...
    LegendaryDescriptionDefault: function () {
        mon.legendariesDescription = "El " + mon.name.toLowerCase() + " puede llevar a cabo 3 acciones legendarias, a elegir de entre las opciones presentadas a continuación. Solo es capaz de utilizar una acción legendaria a la vez, y únicamente al final del turno de otra criatura. El " + mon.name.toLowerCase() + " recupera las acciones legendarias gastadas al comienzo de su turno.";
    },

    // Return the default mythic description
    MythicDescriptionDefault: function () {
        mon.mythicDescription = "Si el rasgo [???] del " + mon.name.toLowerCase() + " se ha activado en la última hora, puede utilizar las opciones siguientes como acciones legendarias.";
    },
...
var StringFunctions = {
    ...
    GetArmorData: function () {
        ...
        if (mon.armorName == "armadura de mago") {
            let mageAC = MathFunctions.GetAC(mon.armorName);
            return mageAC + " (" + (mon.shieldBonus > 0 ? "escudo, " : "") + (mageAC + 3) + " con _armadura de mago_)";
        }
        if (mon.armorName == "none")
            return MathFunctions.GetAC(mon.armorName) + (mon.shieldBonus > 0 ? " (escudo)" : "");
    ...
    GetSpeed: function () {
        ...
        let speedsDisplayArr = [mon.speed + " pies"];
        if (mon.burrowSpeed > 0) speedsDisplayArr.push("excavar " + mon.burrowSpeed + " pies");
        if (mon.swimSpeed > 0) speedsDisplayArr.push("nadar " + mon.swimSpeed + " pies");
        if (mon.climbSpeed > 0) speedsDisplayArr.push("trepar " + mon.climbSpeed + " pies");
        if (mon.flySpeed > 0) speedsDisplayArr.push("volar " + mon.flySpeed + " pies" + (mon.hover ? " (levitar)" : ""));
    ...
    GetPropertiesDisplayArr: function () {
        ...
        if (understandsLanguages.length > 0) {
            if (understandsLanguages.length > 1) {
                if (understandsLanguages.length > 2) {
                    languageDisplayArr.push("entiende " + understandsLanguages[0].name);
                    for (let index = 1; index < understandsLanguages.length; index++)
                        languageDisplayArr.push(understandsLanguages[index].name);
                    languageDisplayArr[languageDisplayArr.length - 1] = " y " + languageDisplayArr[languageDisplayArr.length - 1];
                }
                else
                    languageDisplayArr.push("entiende " + understandsLanguages[0].name + " y " + understandsLanguages[1].name);
            }
            else
                languageDisplayArr.push("entiende " + understandsLanguages[0].name);
            if (mon.understandsBut && mon.understandsBut.trim().length > 0)
                languageDisplayArr[languageDisplayArr.length - 1] += " pero " + mon.understandsBut.trim();
        }

        if (mon.telepathy > 0)
            languageDisplayArr.push("telepatía " + mon.telepathy + " pies");
        ...
        pushArr("Tiradas de salvación", sthrowsDisplayArr);
        ...
        pushArr("Idiomas", languageDisplayArr);
    ...
    MakeTraitHTML: function (name, description) {
        return "<div class=\"property-block\"><div><h4>" + StringFunctions.RemoveHtmlTags(name) + (name ? ".</h4><p> " : "</h4><p> ") + this.FormatString(StringFunctions.RemoveHtmlTags(description), true) + "</p></div></div> <!-- property block -->";
    },
    ...
    MakeTraitHTMLMythic: function (name, description) {
        return "<div class=\"property-block mythic\"><div><li>" + this.FormatString(StringFunctions.RemoveHtmlTags(description), true) + "</li></div></div> <!-- property block -->";
    },
...
var MathFunctions = {
    ...
    GetAC: function (armorNameCheck) {
        let armor = data.armors[armorNameCheck],
            dexBonus = MathFunctions.PointsToBonus(mon.desPoints);
        if (armor) {
            if (armor.type == "ligera") return armor.ac + dexBonus + mon.shieldBonus;
            if (armor.type == "media") return armor.ac + Math.min(dexBonus, 2) + mon.shieldBonus;
            if (armor.type == "pesada") return armor.ac + mon.shieldBonus;
            if (armorNameCheck == "armadura natural") return 10 + dexBonus + mon.natArmorBonus + mon.shieldBonus;
            if (armorNameCheck == "otro") return "otro";
        }
